    ...
    <div class="container">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-6">
                        <h2><b>Stakeholder Locations</b></h2>
                    </div>
                    <div class="col-sm-6">
                        <a href="#addEmployeeModal" class="btn btn-success" data-toggle="modal"><i class="material-icons">&#xE147;</i> <span>Assign new Location</span></a>
                    </div>
                </div>
            </div>
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        @php
                        $stakeholderName = $stakeholder->firstWhere('StakeholderID', $stakeloc->StakeholderID)->Workdep ?? '';
                        $locationName = $location->firstWhere('LocationID', $stakeloc->LocationID)->LocationName ?? '';
                        @endphp
                        <td>{{ $stakeholderName }}</td>
                        <td>{{ $locationName }}</td>
                        <td>
                            <a href="#deleteEmployeeModal" class="delete" data-toggle="modal" data-id="{{ $stakeloc->StakeholderLocationID }}">
    ...
    <div id="addEmployeeModal" class="modal  fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{url('stakeholderLocations')}}" method="POST">
                    ...
                    <div class="form-group">
                        <label for="BossID">Stakeholder</label>
                        <select id="BossID" name="StakeholderID" class="form-control" required>
                            @foreach ($stakeholder as $stek)
                            <option value="{{$stek->StakeholderID}}">{{$stek->StakeholderID}}-> {{$stek->Workdep}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="BossID">Location</label>
                        <select id="BossID" name="LocationID" class="form-control" required>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        deleteButtons.forEach(function(button) {
            button.addEventListener('click', function() {
                ...
                form.action = '/stakeholderLocations/' + id; // This sets the form action URL to /ses/{id}
